...
import { canAccessCreatorFeatures, manualInvoiceLimit, getPlanLimitsText } from "@/src/lib/entitlements";
...
export default function Dashboard() {
  ...
  // Check if user can access invoice creation
  if (invoiceLimit === 0) {
        ...
        <div className='md:w-5/6 w-full h-full p-6'>
          <div className="mb-6">
            <h2 className='font-bold text-2xl mb-2'>
              {isCreator ? 'Creator Dashboard - Add New Invoice' : 'Add new invoice'}
            </h2>
            {invoiceLimit !== 'unlimited' && (
              <p className="text-sm text-gray-600 bg-blue-50 px-3 py-2 rounded-md">
                Starter plan • up to {invoiceLimit} invoices/month • {getPlanLimitsText(plan)}
              </p>
            )}
            {isCreator && (
              <p className="text-sm text-green-600 bg-green-50 px-3 py-2 rounded-md mt-2">
                Creator features: Unlimited invoices, advanced reporting, custom branding
              </p>
            )}
          </div>
